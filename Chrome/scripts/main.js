...
function makeItReal(doItAgain){
    ...
    var msg = chrome.runtime.sendMessage({request: "VuObservationsNeededVu"}, function(msg){
        var direction = ( msg.response.VuObservationsVu.obsLanguage == "AR" ) ? "rtl":"ltr" ;
        for (var i = 0, l = stus.length; i < l; i++){
            var mrk = findMark(stus[i], semester);
            var theObs = getObservation(msg.response, mrk);
